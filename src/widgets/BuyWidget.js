...
const BuyButton = ({ selectedStock }) => {
    ...
    return (
        <div>
            <span>Current Stock: {selectedStock}</span><br></br>
            <span>Current Price: {price}</span>
            <div class="button-container">
                <button class="button buy" onClick={(event) => handleBuySellClick(event, "buy")}>Buy</button>
                <button class="button sell" onClick={(event) => handleBuySellClick(event, "sell")}>Sell</button>
            </div>
            {isModalOpen && (
                <div
                className="modal"
                ...
                <div className="modal-box">
                    ...
                    {modalContent === "buy" ? (
                        <div className="buy-section">
                            <span>Trading: {selectedStock}</span><br></br>
                            <span className="price-span">{price}</span> <span className={isPositive ? "price-change-positive" : "price-change-negative"}>({stock.change}/{stock.percentageChange})</span>
                            ...
                            <div>
                                ...
                                <div className={`quantity-input-container ${orderType === "limit" ? "" : "hidden"}`}>
                                <label htmlFor="quantity" className="quantity-label">
                                    Enter Limit Price:
                                </label>
                                <input
                            ...
                            {quantityType === "dollars" && (
                                <div>
                                    <span>Est.Shares Amt: {orderType === "limit" ? (quantity / limitPrice).toFixed(2) : (quantity / stock.price).toFixed(2)}</span>
                                </div>
                            )}

                            <button className="button complete-order" onClick={() => handleCompleteClick("buy")}>Complete Order</button>
                        </div>
                    ) : (
                            <div className="sell-section">
                                <span>Trading: {selectedStock}</span><br></br>
                                <span className="price-span">{price}</span> <span className={isPositive ? "price-change-positive" : "price-change-negative"}>({stock.change}/{stock.percentageChange})</span><br></br>
                                <span className="price-span">Owned: {samplePnLData.find(stock => stock.ticker === selectedStock).quantity}</span>
                                {/* Quantity Type Toggle Buttons */}
                                <div className="order-type-toggle">
                                <button
                                    className={`order-btn ${quantityType === "shares" ? "active" : ""}`}
                                ...
                                <input
                                    ...
                                    value={quantity}
                                    min={0}
                                    max={quantityType === "shares" ? samplePnLData.find(stock => stock.ticker === selectedStock).quantity : samplePnLData.find(stock => stock.ticker === selectedStock).quantity * price}
                                    onChange={(e) => setQuantity(e.target.value)}
                                    className="quantity-input"
                                    placeholder="Enter quantity"
                                />
                                ...
                                <button className="button complete-order" onClick={() => handleCompleteClick("sell")}>Complete Order</button>
                        </div>
                    )}
